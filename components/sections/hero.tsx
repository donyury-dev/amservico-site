  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-slate-900">
      <div className="absolute inset-0 opacity-20">
        <div className="absolute inset-0 bg-gradient-to-br from-[var(--color-primary,#1E88E5)] to-slate-900" />
      </div>

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-[var(--color-secondary,#10B981)] text-xs font-bold tracking-wider uppercase border border-white/10">
              {badge}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href={primaryCtaHref}
                className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-lg bg-[var(--color-primary,#1E88E5)] text-white font-semibold hover:bg-[var(--color-primary-dark,#0A2540)] transition-colors"
              >
                {primaryCta}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4">
              {highlights.map((h) => (
                <div key={h.label} className="border-l-2 border-[var(--color-primary,#1E88E5)] pl-4">
              ...
              {images.map((img, idx) => (
                ...
                >
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    <div className="text-[var(--color-secondary,#10B981)] text-sm font-bold uppercase tracking-wider">
                      {img.label}
                    </div>
            ...
            <div className="flex justify-center gap-2 mt-4">
              {images.map((_, idx) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${idx === current ? "bg-[var(--color-primary,#1E88E5)] w-6" : "bg-white/40"}`}
